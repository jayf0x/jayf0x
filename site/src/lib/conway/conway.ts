// Conway's Game of Life — self-contained engine
...
const WGSL_RENDER = /* wgsl */ `
  const BLUES: array<vec3f, 9> = array<vec3f, 9>(
  ...
  fn fs(@builtin(position) fc: vec4f) -> @location(0) vec4f {
    ...
    let c = BLUES[min(cnt[i], 8u)] / 255.0;
...
export function createConwayEngine(
  ...
  // ── shared grid state ─────────────────────────────────────────────────────
  let cols = 0, rows = 0, cell = 0, total = 0
  let current = new Uint8Array(0)
  let scratch = new Uint8Array(0)
  let counts = new Uint8Array(0)
  let paused = false, lastStep = 0, raf = 0
  ...
  const cpuStep = () => {
    ...
    for (let i = 0; i < total; i++) {
      const c = counts[i]
      scratch[i] = current[i] ? (c === 2 || c === 3 ? 1 : 0) : c === 3 ? 1 : 0
    }
    const tmp = current
    current = scratch
    scratch = tmp
  ...
  const cpuDraw = () => {
    ...
    for (let ci = 0; ci < 9; ci++) {
      ...
      for (let j = 0; j < b.length; j += 2)
        ctx2d.fillRect(b[j] * cell + 1, b[j + 1] * cell + 1, cell - 2, cell - 2)
      b.length = 0
    }
    if (hovered) {
      const { x, y } = hovered
  ...
  const initGPU = async () => {
    ...
    try {
      ...
      if (destroyed) { gpuDevice.destroy(); gpuDevice = null; return }
      ...
      renderPipeline = gpuDevice.createRenderPipeline({
        ...
        fragment: { module: renderModule, entryPoint: "fs", targets: [{ format: gpuFormat }] },
  ...
  const onDown = (e: MouseEvent) => {
    ...
    if (gpuDevice) {
      ...
      if (curBuf) gpuDevice.queue.writeBuffer(curBuf, i * 4, new Uint32Array([1]))
    }
  }
